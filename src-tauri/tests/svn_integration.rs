//! Integration tests against a real SVN working copy.
//!
//! Run (optional):
//!   set COPY_DIFF_SVN_WC_PATH=C:\path\to\svn\wc
...
fn reader_from_env() -> Option<SvnReader> {
    let wc_path = env("COPY_DIFF_SVN_WC_PATH").or_else(|| env("COPY_DIFF_SVN_URL"))?;
    Some(SvnReader {
        wc_path,
...
#[test]
#[ignore = "requires COPY_DIFF_SVN_WC_PATH and network access to SVN"]
fn list_recent_from_live_repo() {
    let reader = reader_from_env().expect("set COPY_DIFF_SVN_WC_PATH");
