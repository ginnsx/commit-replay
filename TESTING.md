...
| ID | 场景 | 输入 | 预期 |
|---|---|---|---|
| F01 | Git -> Git 单提交 | `DS-GG` 选择 `C02` | 从目标默认分支创建 `relay/时间戳` 分支并新增 1 个 Relay 提交；默认分支 HEAD 不变，文件内容完全匹配 |
| F02 | SVN -> Git 单 revision | `DS-SG` 选择 `C02` | 从目标默认分支创建 `relay/时间戳` 分支并新增 1 个 Git 提交 |
...
| F08 | 失败回滚 | 人为制造一个执行中必失败文件 | 失败后目标回到 baseline，无半应用文件；Git 目标切回默认分支并删除失败分支 |
| F09 | 空净变更 | 选择 `C09` + `C10` | 不产生多余文件；提交行为需符合产品定义并稳定 |
| F10 | 目标工作区已有期望内容 | 目标提前应用同样内容 | 不重复写入，不新增 Relay 提交，Git 目标不创建空分支 |
| F11 | 上下文漂移自动合并 | 目标文件行号漂移但 old block 唯一 | 用户确认 review 后目标内容为合并结果 |
| F12 | 目标已包含跳过 | 目标提前应用同样内容 | 目标文件保持不变，不新增 Relay 提交，Git 目标不创建空分支 |
...
| H02 | 记录详情 | 打开本次记录 | 显示源/目标、实际迁移目标分支、提交、文件、冲突数量、状态 |
| H03 | 文件 diff 回看 | 在记录详情选择文件 | diff 与迁移时预览一致 |
| H04 | 失败迁移 | 执行失败 | 不应写入 success 记录；如有失败记录，状态必须明确为失败 |
| H05 | 本次文件对比 | 成功迁移后点击“对比本次文件”，或在历史详情打开“源 / 目标对比” | 对本次涉及的每个 source/target 路径读取当前工作副本；自定义映射仍比较正确文件；不更新或修改任一仓库 |
| H06 | 对比差异口径 | H05 中包含文本、CRLF/BOM、二进制和删除文件 | 原始字节一致为一致；仅 BOM、换行或单个末尾换行不同标记格式差异；二进制展示 SHA-256；两侧均不存在标记删除一致 |
| H07 | 旧记录安全回退 | 打开升级前、未保存 source/target 配对的记录 | 仅原始 source 路径与记录路径一对一相同时允许对比；映射无法证明时提示无法安全对比，不能按当前映射猜测 |
...
| ID | 场景 | 预期 |
|---|---|---|
| I01 | 所选提交只改 1 个文件 | 目标只出现这 1 个文件变更；Git 目标提交位于新的 `relay/*` 分支且默认分支 HEAD 不变 |
...
| I08 | 执行中失败 | 目标仓库回滚到执行前 baseline；Git 目标不存在遗留的 `relay/*` 失败分支 |
...
| Git -> Git 人工抽检 | A05, A06, B01, C01, D01-D11, E01-E10, F01, F05-F10, G04-G05, H01-H07, I01-I08 |
| SVN -> Git 人工抽检 | A02, B01, C02, D01-D08, F02, F05, H01, H05-H06 |
| Git -> SVN 人工抽检 | A01, B01, C01, F03, F05, H01, H05-H06 |
| SVN -> SVN 人工抽检 | A02, B01, C02, F04, F05, H01, H05-H06 |
